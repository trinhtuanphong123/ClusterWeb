...
## Goal
Build a correlation/similarity graph over the window and apply Leiden community detection, writing edges to `cluster_edges` and communities to `cluster_runs`/`cluster_members`. Window-recomputed, nightly.
...
- `pipelines/common/` (graph helpers)
...
## Notes for agent
Window-recomputed, nightly. Large matrices go to S3 via `model_artifacts`. Keep edges canonical to avoid duplicates.
